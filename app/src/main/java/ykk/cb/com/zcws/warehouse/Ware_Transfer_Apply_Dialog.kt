...
import kotlinx.android.synthetic.main.ware_transfer_apply_dialog.*
import okhttp3.*
 ...
 * 调拨申请
 */
class Ware_Transfer_Apply_Dialog : BaseActivity() {
    ...
    private class MyHandler(activity: Ware_Transfer_Apply_Dialog) : Handler() {
        private val mActivity: WeakReference<Ware_Transfer_Apply_Dialog>
        ...
        override fun handleMessage(msg: Message) {
            ...
            if (m != null) {
                ...
                when (msg.what) {
                    ...
                    SUCC2 -> { // 查询物料 成功
                        val list = JsonUtil.strToList(msgObj, ICItem::class.java)
                        m.getMtlAfter(list)
                    }
                    UNSUCC2 -> { // 查询物料    数据加载失败！
                        m.tv_icItemName.text = ""
    ...
    override fun setLayoutResID(): Int {
        return R.layout.ware_transfer_apply_dialog
    }
...
//        mHandler.sendEmptyMessageDelayed(SETFOCUS, 200)
    ...
    private fun bundle() {
        ...
        if(bundle != null) {
            val strMtlId = bundle.getString("strMtlId")
            run_findMtlById(strMtlId)
        }
    }
...
//        et_positionCode.setOnClickListener(click)
     ...
     * 通过okhttp加载数据物料信息
     */
    private fun run_findMtlById(strMtlId :String) {
        val formBody = FormBody.Builder()
                .add("strMtlId", strMtlId)
                .add("stockQtyGt0","0")
                .build()
        showLoadDialog("加载中...", false)
        val mUrl = getURL("material/findListByParam2")
        ...
        call.enqueue(object : Callback {
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                Log.e("run_findMtlById --> onResponse", result)
                mHandler.sendMessage(msg)
            }
        })
    }
