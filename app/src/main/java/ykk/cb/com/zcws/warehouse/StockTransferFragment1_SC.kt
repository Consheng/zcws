...
import kotlinx.android.synthetic.main.ware_sc_stock_transfer_fragment1_sc.*
import okhttp3.*
 ...
 * 作者：ykk
 */
class StockTransferFragment1_SC : BaseFragment() {
    ...
    private val checkDatas = ArrayList<Inventory_K3>()
    ...
    private class MyHandler(activity: StockTransferFragment1_SC) : Handler() {
        private val mActivity: WeakReference<StockTransferFragment1_SC>
                ...
                when (msg.what) {
                    m.SUCC1 -> { // 扫码成功 进入
                        when(m.smqFlag) {
                            ...
                            '3' -> {
                                val icInventory = JsonUtil.strToObject(msgObj, Inventory_K3::class.java)
                                var listIcInvBackup = ArrayList<Inventory_K3>()
                    ...
                    m.SUCC2 -> { // 历史查询 进入
                        m.checkDatas.clear()
                        val icInvBackup = JsonUtil.strToList(msgObj, Inventory_K3::class.java)
    ...
    override fun initView() {
        ...
        mAdapter!!.setCallBack(object : StockTransferFragment1_SC_Adapter.MyCallBack {
            override fun onClick_num(entity: Inventory_K3, position: Int) {
                curPos = position
    ...
    fun onViewClicked(view: View) {
        ...
        when (view.id) {
            ...
            R.id.btn_mtlSel -> { // 选择物料
                ...
                if(!checkSelStockPos()) return;
                bundle = Bundle()
                bundle.putInt("stockId", outStock!!.fitemId)
                bundle.putInt("stockPosId", if(outStockPos != null) outStockPos!!.fspId else 0)
                bundle.putString("accountType", "ZH");
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        when (requestCode) {
            ...
            SEL_MTL //查询物料	返回
            -> if (resultCode == Activity.RESULT_OK) {
                val list = data!!.getSerializableExtra("obj") as List<Inventory_K3>
     ...
     * 得到扫码或选择数据
     */
    private fun getMtlAfter(list: List<Inventory_K3>) {
        ...
        for (icInventory in list) {
            ...
            for (i in 0 until size) {
                ...
                // 有相同的，就不新增了
                if (sr.fitemID == icInventory.fitemID) {
                    curPosition = i
                    addRow = false
                    break
                }
    ...
    private fun run_smDatas() {
        ...
        val formBody = FormBody.Builder()
                .add("stockId", outStock!!.fitemId.toString())
                .add("stockPosId", if(outStockPos != null) outStockPos!!.fspId.toString() else "")
    ...
    private fun run_save() {
        ...
        checkDatas.forEachIndexed { index, icInventory ->
            if(icInventory.fqty < icInventory.realQty) {
            ...
            save.icItemId = icInventory.fitemID
            ...
            save.accountName = "生产账号"
            saveList.add(save)
        }
        showLoadDialog("保存中...", false)
        val mUrl = getURL("stockTransferRecord/addList_SC")
    ...
    private fun run_findListByParamWms() {
        isTextChange = false
        showLoadDialog("加载中...", false)
        val mUrl = getURL("Inventory_K3/findListByParamWms")
        val formBody = FormBody.Builder()
